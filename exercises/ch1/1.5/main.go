...
// where each pair of hexadecimal digits represents the intensity
...
func lissajous(out io.Writer) {
	...
	for i := 0; i < nframes; i++ {
		...
		for t := 0.0; t < cycles*2*math.Pi; t += res {
			...
			img.SetColorIndex(size+int(x*size+0.5), size+int(y*size+0.5), greenIndex) // changed from blackIndex to greenIndex
		}
		phase += 0.1
